...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned* heightPtr, unsigned* widthPtr, unsigned *channelsPtr) {
    int  x, y, i, v, u;
    int  R, G, B;
    double val[MASK_N*3] = {0.0};
    int adjustX, adjustY, xBound, yBound;

    unsigned height = *heightPtr;
    unsigned width = *widthPtr;
    unsigned channels = *channelsPtr;
...
int main(int argc, char** argv) {
    ...
    cudaMalloc((void **)&dev_tar ,height * width * channels * sizeof(unsigned char));


    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(dev_h, &height, sizeof(int),cudaMemcpyHostToDevice);
    cudaMemcpy(dev_w, &width, sizeof(int),cudaMemcpyHostToDevice);
    cudaMemcpy(dev_c, &channels, sizeof(int), cudaMemcpyHostToDevice);
    ...
    int blockSize = width * height / 1024;
    dim3 a(1,2);
    sobel<<<height, 1024>>>(host_s, dev_tar, dev_h, dev_w, dev_c);
cudaError_t err;
...
//}
    ...
    cudaMemcpy(host_t, dev_tar, height * width * channels * sizeof(unsigned char), cudaMemcpyDeviceToHost);
