...
__global__ void phase_3(int* dev_Dist, int Round,int total_round, int row_size) {
    ...
    int b_i = (Round+1+blockIdx.x) % total_round;
...
void block_FW(){
    ...
    dim3 p3(total_round-1, total_round-1);


    for (int r = 0; r < total_round; ++r) {
        /* Phase 1*/
        phase_1<<< p1, threads>>>(dev_Dist, r, total_round, row_size);
        ...
        /* Phase 2*/
        phase_2<<< p2, threads>>>(dev_Dist, r, total_round, row_size);
        // err = cudaGetLastError();
        // if ( err != cudaSuccess )
        // {
        ...
        /* Phase 3*/
        phase_3<<< p3, threads>>>(dev_Dist, r, total_round, row_size);
        // err = cudaGetLastError();
        // if ( err != cudaSuccess )
        // {
...
int main(int argc, char* argv[]) {
    ...
    cudaMallocPitch((void **)&dev_Dist, &pitch, padding*sizeof(int), padding);
    row_size = pitch/sizeof(int);
    // printf("pitch is %d\n", pitch);


    // start = omp_get_wtime();
    cudaMemcpy2D(dev_Dist, pitch, Dist, padding * sizeof(int), padding * sizeof(int), padding, cudaMemcpyHostToDevice);
    // end = omp_get_wtime();
    // printf("H2D %fs\n", end-start);
    ...
    // start = omp_get_wtime();
    cudaMemcpy2D(Dist, n*sizeof(int), dev_Dist, pitch, n * sizeof(int), n, cudaMemcpyDeviceToHost);
    // end = omp_get_wtime();
    // printf("D2H %fs\n", end-start);
