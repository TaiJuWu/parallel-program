...
// after calculate two pixels , run this function in order to check pixels
// already statify conditions
  ...
  while (1) {
    ...
    for (int l = 0; l < k_size; l = l + 2) {
      ...
      // cal two pixels
        ...
        for (int x = 0; x < 2; ++x) {
          continue_each_pixel_repeats(image_repeats, k[x], x0[x], y0[x], iters,
                                      x_res[x], y_res[x]);
        }
...
int main(int argc, char **argv) {
  ...
  for (int cpu = 0; cpu < ncpus; ++cpu) {
    ...
    pthread_create(&threads[cpu], NULL, thread_process,
                   (void *)&thread_task[cpu]);
  }
