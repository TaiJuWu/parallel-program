...
void cal_image_color(int iters, int width, int height, int color_channel,
                     int *repeats, png_bytep image, const int k_start,
                     const int k_end) {
...
void continue_each_pixel_repeats(int *image, const int k, const double x0,
                                 const double y0, const int iters,
...
void cal_two_pixels_repeats(int *image, const int *k, const double *x0,
                            const double *y0, const int iters, double *x_res,
                            double *y_res) {
  ...
  } while (_mm_test_all_ones(repeat_comp) &&
           _mm_test_all_ones(_mm_castpd_si128(len_squa_comp)));
...
void write_png(const char *filename, png_bytep image, int width, int height,
               int color_channel) {
...
void cal_each_pixel_repeats(int *image, const int k, const double x0,
                            const double y0, const int iters) {
  // calculate
  int repeats = 0;
  double x = 0;
...
void cal_repeats(int iters, double left, double right, double lower,
                 double upper, int width, int height, int *image,
...
#pragma omp parallel for schedule(dynamic)
  for (int idx = 0; idx < size; ++idx) {
    ...
    for (int x = 0; x < 2; ++x) {
      continue_each_pixel_repeats(image, k[x], x0[x], y0[x], iters, x_res[x],
                                  y_res[x]);
    }
  }
...
void reduceImage(png_bytep image, png_bytep collect, const int pixels,
                 const int color_channel, const int avg_task, const int size,
                 const int k_start, const int k_end) {
  int disp[size];
  int recvCnt[size];
...
#pragma omp parallel for
  ...
  MPI_Gatherv(image + k_start * color_channel, (k_end - k_start) * pixel_size,
              MPI_UNSIGNED_CHAR, collect, recvCnt, disp, MPI_UNSIGNED_CHAR, 0,
              MPI_COMM_WORLD);
}
...
int main(int argc, char **argv) {
  ...
  cal_repeats(iters, left, right, lower, upper, width, height, repeats, k_start,
              k_end);

  png_bytep image = (png_bytep)malloc(height * row_size);
  cal_image_color(iters, width, height, color_channel, repeats, image, k_start,
                  k_end);
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  commStart = MPI_Wtime();
  reduceImage(image, collect, pixels, color_channel, avg_task, size, k_start,
              k_end);
  commEnd = MPI_Wtime();
  free(image);
